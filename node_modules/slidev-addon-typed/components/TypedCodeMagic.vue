...
// Use the steps prop directly
const getCodeSteps = (): string[] => {
  return props.steps.filter(step => step.trim().length > 0);
};
...
onMounted(async () => {
  ...
  // Get code steps from props
  const codeSteps = getCodeSteps();
  if (codeSteps.length === 0) {
    console.warn('No code steps provided to TypedCodeMagic component');
    return;
  }
  ...
  const highlightedStrings = await Promise.all(
    codeSteps.map(code => highlightCode(code))
  );
